...
### Password "vlead" user on ansible/configu server.
...
## Each cluster container will have the following password for root.
...
## No need to edit this line, systems_model_common_vars_path is a location of othe systems model common_vars file.
systems_model_common_vars_path: "~/systems-model/build/{{clustername}}/roles/common_vars/vars"
...
##SMTP server for mail forwarding. Through which mails from each container will be forwarded to destination 
